...
# ADD THESE IMPORTS
from services.site_service import build_site_centroid_df
from services.analytics_service import prepare_daily_alert_trend
...
live_df = (
    site_filtered_defor_df[
        site_filtered_defor_df["source"]
        .astype(str)
        .str.contains("Live API", case=False, na=False)
    ]
    if not site_filtered_defor_df.empty
    and "source" in site_filtered_defor_df.columns
...
if filtered_defor_df.empty:
    ...
    kpis = {
        ...
        "high": int(
            (filtered_defor_df["severity"] == "High").sum()
        ),
        "medium": int(
            (filtered_defor_df["severity"] == "Medium").sum()
        ),
        "low": int(
            (filtered_defor_df["severity"] == "Low").sum()
        ),
    }
